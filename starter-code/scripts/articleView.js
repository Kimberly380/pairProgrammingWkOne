...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if (authorVal) {
      $('article').hide().filter('[data-author="'+ authorVal + '"]').fadeIn('slow').show();
      // WeDidIt: If the select box was changed to an option that has a value, we need to hide all the articles,
      //       and then show just the ones that match for the author that was selected.
      //       Use an "attribute selector" to find those articles, and fade them in for the reader.

    } else {
      $('article').show();
      // WeDidIt: If the select box was changed to an option that is blank, we should
...
articleView.handleCategoryFilter = function() {
  ...
  // WeDidiT: Just like we do for #author-filter above, we should handle change events on the #category-filter element.
...
articleView.handleMainNav = function() {
  ...
  /*
  ----------KIM GOOD LUCK! MAKE THIS DYNAMIC PLEASE-----------------
...
$('.main-nav').on('click',function(){
  ...
  $('.main-nav .tab:first-child').click(function() {
    $('#about').hide();
    $('#articles').show();
  });
